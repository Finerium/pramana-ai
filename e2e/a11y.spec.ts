...
test("AC-A11Y-01 axe nol serious/critical pada 5 layar kunci", async ({ page }) => {
  ...
  expect(laporan, `pelanggaran serious/critical: ${laporan.join(" | ")}`).toEqual([]);
});

test("AC-A11Y-02 touch target >= 44px dan teks nominal >= 24px", async ({ page }) => {
  await login(page, "anggota");
  await page.goto("/beranda");
  ...
  expect(box!.height, "touch target aksi utama >= 44px").toBeGreaterThanOrEqual(44);
  ...
  expect(maxNominalFs, "teks nominal utama layar anggota >= 24px").toBeGreaterThanOrEqual(
    24,
  );
});
