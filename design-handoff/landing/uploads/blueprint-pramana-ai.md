...
Input wajib: tiga arsip di root proyek saat launch, nama persis pramana-ai_mobile.zip (aplikasi anggota), pramana-ai_dashboard.zip (dasbor pemerintah), dan pramana-ai_landing-page.zip (landing page), hasil ekspor Code Handoff Bundle Claude Design. Phase 0 memverifikasi ketiganya, meng-unzip ke design-handoff/mobile/, design-handoff/dashboard/, dan design-handoff/landing/, lalu menulis inventaris ke .crown/design-inventory.md (pemetaan tiap layar bundle ke fitur F01-F15, daftar token, state yang tercakup, catatan README bundle). Salah satu absen atau korup = STOP and ask. design-handoff/ read-only bagi worker (oracle, setara fixtures).

Aturan fidelity:
1. Token bundle diport verbatim ke lapisan token aplikasi (warna OKLCH/hex, radius, spacing, tipografi, shadow). Deviasi nilai hanya sah sebagai improvement terdokumentasi.
...
6. Konflik antar bundle (misal nilai token verdict berbeda): satukan pada satu nilai kanonik dengan urutan prioritas mobile, lalu landing, lalu dashboard (mobile adalah permukaan utama juri), catat sebagai improvement.
...
- Q1 Bundle desain absen atau korup saat Phase 0? STOP and ask. Tidak ada jalur desain interim; ketiga zip adalah prasyarat launch dan operator memastikannya sebelum menjalankan.
...
Catatan status: tiga prompt Claude Design telah diautorkan sebagai sesi terpisah (anggota, pemerintah, landing page); hasil ekspornya masuk sebagai pramana-ai_mobile.zip, pramana-ai_dashboard.zip, dan pramana-ai_landing-page.zip, diperlakukan per kontrak 6.19.
